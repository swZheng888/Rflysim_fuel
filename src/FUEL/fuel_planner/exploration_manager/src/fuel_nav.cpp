...
Ctrl::Ctrl()
{
    ...
    state_sub = nh.subscribe("/uav0/mavros/state", 10, &Ctrl::state_cb, this);
    position_sub=nh.subscribe("/odom", 10, &Ctrl::position_cb, this);
    target_sub = nh.subscribe("move_base_simple/goal", 10, &Ctrl::target_cb, this);
    twist_sub = nh.subscribe("/drone_0_planning/pos_command", 10, &Ctrl::twist_cb, this);
    local_pos_pub = nh.advertise<mavros_msgs::PositionTarget>("/uav0/mavros/setpoint_raw/local", 1);
